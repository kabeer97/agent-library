...
export const UseCaseCard: React.FC<UseCaseCardProps> = ({ title, badges, description, integrations }) => {
  ...
  if (title === "Align Documents to New Accounting Standards") {
    return (
      <Link to="/align-documents-to-new-accounting-standards" className="block">
        {cardContent}
      </Link>
    );
  }

  if (title === "Train New Employees on Finance Processes") {
    return (
      <Link to="/train-new-employees-on-finance-processes" className="block">
        {cardContent}
      </Link>
    );
  }
